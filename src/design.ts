...
export async function runPlanner(o: DesignOptions): Promise<Plan | null> {
  ...
  const basePrompt = [
    ...
    `   it can be caught.`,
    `4. \`needs\`: ONLY when a task needs a capability for a reason that is not`,
    `   one of its gates — gate requirements are derived automatically, so this`,
    `   is the exception. Each entry is {capability, because}, and \`because\``,
    `   must say why. It is a REQUEST, never a grant: the operator's config`,
    `   decides whether any elevation is permitted, and no flag overrides that.`,
    `5. \`covers\`: which charter objectives this task advances, by exact text.`,
    ...
    `6. \`dependsOn\`: only where a task genuinely needs another's result to exist.`,
    `   Spurious dependencies serialise the whole arc.`,
    `7. \`acceptance\`: at least one criterion per task, each naming its OWN proof.`,
