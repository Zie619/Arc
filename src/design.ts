...
export async function runPlanner(o: DesignOptions): Promise<Plan | null> {
  ...
  const basePrompt = [
    ...
    `2. \`footprint\`: the files the task will edit. Be honest and complete —`,
    ...
    `3. \`contractsMutated\` / \`contractsRead\`: the REAL scheduling unit. An`,
    ...
    `   it can be caught.`,
    `4. \`dependsOn\`: only where a task genuinely needs another's result to exist.`,
    `   Spurious dependencies serialise the whole arc.`,
    `5. \`acceptance\`: at least one criterion per task, each naming its OWN proof.`,
    ...
    ``,
    `Prefer FEWER, larger tasks over many tiny ones — every task carries a`,
    `dispatch, a review and a land. Aim for the smallest set that covers the`,
    `objectives.`,
    ``,
    `## Goal`, charter.goal,
